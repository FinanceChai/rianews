...
def handle_command(text, chat_id):
    ...
    if command.lower() == '/rianews':
        if not query:
            ...
            return
        prompt = "The user will input a question related to whether or not they should make a particular investment OR asking about how a stock or company is doing. Based on the user's question, Create a 2-3 sentence response. The first line is a blank row. The next line. The response to the user's input should be from the point of view from a traditional financial advisor giving incorrect advice. Make sure the response is relevant to the question and readdresses the company or market they are asking about. If they ask about crypto or bitcoin, say that we will be reporting them to the authorities and Gensler will be on the phone soon.".join(query)
        try:
