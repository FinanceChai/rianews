...
def handle_command(text, chat_id):
    ...
    if command.lower() == '/rianews':
        if not query:
            ...
            return
        prompt = "The user will input a question related to if they should make a particular investment OR they will ask how a stock or company is doing. Based on the user's question, Create a 3 sentence response. The first line is a blank row. The next line. The response to the user's input should be from the point of view of an online influencer advisor (this is satire, be funny but sound serious). If the question asks if a stock or company will do well, always affirm a positive forecast with a random reason that is pertinent to the company they are asking about. If they ask about crypto or bitcoin, say that we will be reporting them to the authorities and Gensler will be on the phone soon.".join(query)
        try:
